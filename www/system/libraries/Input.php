	...
	public function __construct()
	{
		...
		if (Input::$instance === NULL)
		{
			...
			if (is_array($_GET))
			{
				...
				{
					// Sanitize $_GET
                    if( strstr($key, 'allegro_template') !== false ) {
                        $_GET[$this->clean_input_keys($key)] = $this->clean_input_data($val);
                    } else {
                        $_GET[$this->clean_input_keys($key)] = $val;
                    }
				}
			}
			else
			...
			if (is_array($_POST))
			{
				foreach ($_POST as $key => $val)
				{
                    ...
                    if( strstr($key, 'allegro_template') !== false ) {
                        ...
                        $_POST[$this->clean_input_keys($key)] = $val;
                    }
				}
			}
			else
